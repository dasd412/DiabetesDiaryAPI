...
public class CustomUserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
  ...
  public OAuth2User loadUser(OAuth2UserRequest oAuth2UserRequest)
    ...
    String registeredId = oAuth2UserRequest.getClientRegistration()
        .getRegistrationId();//현재 로그인 중인 서비스를 구분하기 위함. 네이버인지 구글인지 구분!

    String userNameAttributeName = oAuth2UserRequest.getClientRegistration().getProviderDetails()
        .getUserInfoEndpoint().getUserNameAttributeName();//로그인하는 id의 PK를 가져온다.
    ...
    return new DefaultOAuth2User(
        Collections.singleton(new SimpleGrantedAuthority(writer.getRoleKey())),
